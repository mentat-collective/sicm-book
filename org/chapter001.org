...
* 1.2 Generalized Coordinates
  ...
  coordinate function /χ/ that maps elements of the configuration space to
  /n/-tuples of real numbers.[fn:5] If there is more than one dimension, the
  function /χ/ is a tuple of /n/ independent coordinate functions[fn:6] /χ^{i}, i/
  ...
  a /coordinate path q/ = /χ/ ∘ /γ/ mapping time to tuples of generalized
  coordinates.[fn:9] If there is more than one degree of freedom the coordinate
  path is a structured object: /q/ is a tuple of component coordinate path
  functions /q^{i}/ = /χ^{i}/ ∘ /γ/. At each instant of time /t/, the values
  /q/(/t/) = (/q/^{0}(/t/), ..., /q/^{/n/−1}(/t/)) are the generalized
...
;;=> (+ (* 1/2 m (expt ((D x) t) 2)) (* 1/2 m (expt ((D y) t) 2)) (* 1/2 m (expt
;;((D z) t) 2)))
  #+end_src
...
** Paths of minimum action
   ...
   test path with action /S/[/q/](/t/_{1}, /t/_{2}). Let /q/ + /ϵη/ be a nearby
   path, obtained from /q/ by adding a path variation /η/ scaled by the real
   parameter /ϵ/.[fn:37] The action on the varied path is /S/[/q/ + /ϵη/](/t/_{1},
   /t/_{2}). Euler and Lagrange found that /S/[/q/ + /ϵη/](/t/_{1}, /t/_{2}) >
   /S/[/q/](/t/_{1}, /t/_{2}) for any /η/ that is zero at the endpoints and for
   any small nonzero /ϵ/.

   Let's check this numerically by varying the test path, adding some amount of
   a test function that is zero at the endpoints /t/ = /t/_{1} and /t/ =
   /t/_{2}. To make a function /η/ that is zero at the endpoints, given a
   sufficiently well-behaved function /ν/, we can use /η/(/t/) = (/t/ −
   /t/_{1})(/t/ − /t/_{2})/ν/(/t/). This can be implemented:
...
** Lagrange equations
   ...
   $\begin{array}{ll} {\frac{d}{dt}\frac{\partial
   L}{\partial{\overset{˙}{q}}^{i}} - \frac{\partial L}{\partial q^{i}} = 0} &
   {i = 0,\ldots,n - 1.} \\ \end{array}$
   ...
   \end{array}} \right) - \left. \frac{\partial L\left( {t,w,\overset{˙}{w}}
...
** A Direct Derivation
   ...
   first order in /ϵ/, /ϵDg/(0). Because /η/ is zero at the endpoints the
   ...
   choice of the variation that the factor in the integrand multiplying /η/ is
   ...
   to be nonzero in at least one of its components. But if we choose our /η/ to
...
** The Variation Operator
   ...
   /q/ + /ϵη/ be a varied path, where the function /η/ is a path-like function
   ...
   /f/ for small variations in the path. The variation of /f/ depends on /η/.
   ...
   $$D\begin{array}{ll} {\delta_{\eta}f\lbrack q\rbrack = \delta_{\eta}g\lbrack
   ...
   particular function /η/ to /q/. So the statement /δ_{η}f/[/q/] = 0 for
   arbitrary /η/ implies the function /f/ is stationary for small variations of
   the path around /q/.
...
** A Derivation with the Variation Operator
   ...
   variation /η/ that preserves the endpoints, /η/(/t/_{1}) = /η/(/t/_{2}) = 0,
   is zero:
   ...
   For our variation /η/ we have /η/(/t/_{1}) = /η/(/t/_{2}) = 0, so the first
   term vanishes.
   ...
   realizable path. Thus ([[file:chapter001.html#disp_1.34][1.34]]) must be true for /any/ function /η/ that is zero
   at the endpoints. Since /η/ is arbitrary, except for being zero at the
...
** Harmonic oscillator
   ...
   \begin{array}{ll} {mD^{2}y(t) + ky(t) = 0,} & {(1.41)} \\ \end{array}
...
** Orbital motion
   ...
   where /ξ/ and /η/ are formal parameters for rectangular coordinates of the
...
* 1.6 How to Find Lagrangians
  ...
  *x*_{/N/−1}(/t/)). The Lagrange equations for the coordinate path /x/ are
  ...
  \begin{array}{ll} {\partial_{2}L(t;\mathbf{x}_{0},\ldots,\mathbf{x}_{N -
...
(show-expression (((Lagrange-equations
   ...
   y(t)}{\sqrt{{(x\,\,(t))}^{2} + {(y\,\,(t))}^{2}}}} \\ \end{array}
   \right\rbrack$$
   ...
   between rectangular coordinates (/x/, /y/) and polar coordinates (/r, φ/) is
   ...
   We express this Lagrangian as follows:
    ...
    #+begin_src scheme
(define ((L-central-polar m U) local) (let ((q
   (coordinate local)) (qdot (velocity local))) (let ((r (ref q 0)) (phi (ref q
   1)) (rdot (ref qdot 0)) (phidot (ref qdot 1))) (- (* 1/2 m (+ (square rdot)
   (square (* r phidot))) ) (U r)))))
    #+end_src
   ...
   Lagrange's equations are
    ...
    #+begin_src scheme
(show-expression (((Lagrange-equations
   (L-central-polar 'm (literal-function 'U))) (up (literal-function 'r)
   (literal-function 'phi))) 't))
    #+end_src

   $$\left\lbrack \begin{array}{l} {mD^{2}r\,(t) -
   mr\,(t)\,{(D\varphi\,\,(t))}^{2} + DU\,(r\,(t))} \\ {2mDr\,(t)\, r\,(t)\,
   D\varphi\,(t) + mD^{2}\varphi\,(t)\,{(r\,(t))}^{2}} \\ \end{array}
   \right\rbrack$$
...
* 1.7 Evolution of Dynamical State
  ...
  \partial_{0}\partial_{2}L \circ \Gamma\lbrack q\rbrack\rbrack} \\ \end{array}$
...
* 1.10 Constrained Motion
  ...
  At this point we argued that because the variations /η/ are arbitrary (except
  for conditions at the endpoints), the only way for the integral to be zero is
  for the integrand to be zero. Furthermore, the freedom in our choice of /η/
  allowed us to deduce that the factor multiplying /η/ in the integrand must be
  identically zero, thereby deriving Lagrange's equations.

  Now the choice of /η/ is not completely free. We can still deduce from the
  arbitrariness of /η/ that the integrand must be zero,[fn:91] but we can no
  longer deduce that the factor multiplying /η/ is zero (only that the
  ...
  variations /η/ for which the variation of the constraint is zero:
  ...
  Expanding this with the chain rule, a variation /η/ is tangent to the
...
*** 1.10.1 Coordinate Constraints
    ...
    do we eliminate /η/? The residual of the Lagrange equations is
    orthogonal[fn:92] to any /η/ that is orthogonal to the normal to the
...
** Alternatively
   ...
   \begin{array}{ll} {\mathcal{E}\lbrack L''\rbrack \circ \Gamma\lbrack q\rbrack}
...
** Building systems from parts
   ...
   an additional configuration coordinate /ξ/. We can formulate a Lagrangian for
   each part separately. We can then choose a Lagrangian for the composite
   system as the sum of the two component Lagrangians with a constraint /ξ/ =
   ...
   together. We can now eliminate the “glue” coordinates /ξ/ and /λ/ to obtain
...
*** 1.10.3 Nonholonomic Systems
    ...
    $$(\begin{array}{ll} {\mathcal{E}\lbrack L\rbrack \circ \Gamma\lbrack
    q\rbrack)\eta = 0,} & {(1.232)} \\
    \end{array}

    and

    $$(\begin{array}{ll} {\partial_{1}\varphi \circ \Gamma\lbrack q\rbrack)\eta
    = 0.} & {(1.233)} \\
    \end{array}
    ...
    is orthogonal to /η/ and /η/ is constrained to be orthogonal to ∂_{1}/φ/ ∘
    ...
    velocity. In this case, for a variation /η/ to be consistent with the
    ...
    We may no longer eliminate /η/ by the same argument, because /η/ is no
    ...
    al. [[[file:bibliography.html#bib_6][6]]], translated into our notation. Define a “virtual velocity” /ξ/ to be
    any velocity satisfying

    $$(\begin{array}{ll} {\partial_{2}\psi \circ \Gamma\lbrack q\rbrack)\xi =
    0.} & {(1.236)} \\
    \end{array}
    ...
    required to be orthogonal to ∂_{2}/ψ/ ∘ Γ[/q/] and any such /ξ/ is
    orthogonal to
    ...
    must replace /η/ by /ξ/. However, to get from equation ([[file:chapter001.html#disp_1.235][1.235]]) to equation
    ([[file:chapter001.html#disp_1.236][1.236]]), we must set /η/ = 0 and replace /Dη/ by /ξ/. All “derivations” of
    the nonholonomic equations have similar identifications. It comes down to
    this: the nonholonomic equations do not follow from the action principle.
    They are something else. Whether they are correct or not depends on whether
    or not they agree with experiment.
...
[fn:9] Here ∘ denotes composition of functions:
